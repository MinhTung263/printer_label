...
class MainActivity: FlutterPlugin, MethodCallHandler {
  ...
  private val printLabel : PrintLabel = PrintLabel()
...
//    printLabel = PrintLabel()
  ...
  override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
    when (call.method) {
      ...
      "connect_usb" -> {
        printLabel.actitonConnectUSB()
      }
      "connect_lan" -> {
        val ipAddress = call.argument<String>("ip_address")
        if(!ipAddress.isNullOrEmpty()){
          printLabel.connectNet(ipAddress)
        }
      }
      "print_barcode" -> {
        printLabel.printBarcode(call,result)

      }
      "print_image" -> {
        printLabel.printImage(call)
